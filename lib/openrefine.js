...
module.exports = function (endpoint) {
  ...
  server.upload = function (file_name) {
    ...
    server._file_name = file_name
    return sa.post(conf.endpoint + '/command/core/create-project-from-upload?options={"encoding":"UTF-8","separator":",","ignoreLines":-1,"headerLines":1,"skipDataLines":0,"limit":-1,"storeBlankRows":true,"guessCellValueTypes":false,"processQuotes":true,"storeBlankCellsAsNulls":true,"includeFileSources":false}')
      .use(promise())
      .redirects(0)
      .field('project-name', server._project_name)
      .attach('project-file', fs.readFileSync(server._file_name), server._file_name)
      .end()
      .catch(err => {
        if (err.status === 302) {
          return err.response
      ...
      .then(res => ({
        project_id: +res.headers.location.replace(conf.endpoint + '/project?project=', '')
      }))
  }
